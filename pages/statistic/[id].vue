      ...
      <div v-if="!isAuth" @click="toggleDropdownAuth"  ref="dropdownAuth" style="position: relative; cursor: pointer; background: #7EEFFF; padding: 0.5rem 1rem; border-radius: 50%; border: none;">
        ...
        <div style="padding: 2rem;">
            <div style="display:flex; justify-content: space-between;">
                ...
                <h2 style="font-size: 3rem; color: #FFFF; margin-bottom: 2rem;   display: flex;
  justify-content: end;
  align-items: end;">
                Statistic
            </h2>
...
<div class="mob-footer-sticky" style="position: sticky; bottom: 0; background: linear-gradient(90deg, #A7D759 0%, #84963C 100%); height: 50px;">
    ...
    const statistics = ref<null | { postCount: number, postData: { date: string, count: number }[] }>(null);


    async function fetchStatistics(userId: string) {
        const startDate = selected.value.start.toISOString();
        const endDate = selected.value.end.toISOString();

        try {
            const response = await fetch(`/api/profile/${userId}?startDate=${startDate}&endDate=${endDate}`);
            if (response.ok) {
                ...
                if (data.statistics) {
                    ...
                    // Update chart data with formatted date
                    chartData.value = {
                        labels: data.statistics.map(item => format(new Date(item.date), 'd MMMM')),  // Format date
                        datasets: [
                            {
                                label: 'Posts',
                                backgroundColor: '#11FFE7',
                                data: data.statistics.map(item => item.postCount),
                            },
                        ],
                    };
                    ...
                    console.warn("Data structure unexpected. Missing 'statistics' in response.");
                    statistics.value = { postCount: 0, postData: [] };
